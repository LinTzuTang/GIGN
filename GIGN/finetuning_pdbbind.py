...
if __name__ == '__main__':
    ...
    for repeat in range(repeats):
        ...
        # Initialize a separate wandb run for each repetition
        wandb.init(project="finetuning_rep", config=args, name=f"repeat_{repeat}")
        wandb.config.update({"model": "GIGN", "fold": args.get('fold'), "repeat": repeat})
        ...
        for epoch in range(epochs):
            ...
            if valid_rmse < running_best_mse.get_best():
                ...
                if count > early_stop_epoch:
                    logger.info(f"Early stopping in epoch {epoch}. Best RMSE: {running_best_mse.get_best():.4f}")
                    break
                
        # Save the last epoch model after completing all epochs or early stopping
        msg = "last_epoch-%d, train_loss-%.4f, train_rmse-%.4f, valid_rmse-%.4f, valid_pr-%.4f" \
              % (epoch, epoch_loss, epoch_rmse, valid_rmse, valid_pr)
        model_path = os.path.join(logger.get_model_dir(), msg + '_last.pt')
        save_model_dict(model, logger.get_model_dir(), msg)

        logger.info("Training finished and last epoch model saved for repeat %d." % repeat)

        # Finish the wandb run for the current repetition
        wandb.finish()
