...
def inter_graph(ligand, pocket, dis_threshold=5.):
    ...
    # Find node indices where the distance between atoms is less than the threshold
    ...
    #     # Return an empty tensor if there are no edges
    #     return torch.LongTensor([])

    # Stack the edges into a tensor if the graph has edges
...
if __name__ == '__main__':
    ...
    data_set = GraphDataset(data_dir, data_df, graph_type='Graph_GIGN', dis_threshold=5, create=True)
    train_loader = PLIDataLoader(data_set, batch_size=128, shuffle=True, num_workers=4)
